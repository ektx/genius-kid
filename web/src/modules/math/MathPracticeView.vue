...
import type { MathOp, MathRange, MathType, QuestionCount } from '../../utils/mathGenerator';
...
const submitAnswer = () => {
  ...
  store.submitAnswer(parseInt(userInput.value));
  userInput.value = '';
...
<template>
  <div class="practice-container">
    ...
    <!-- 1. 设置区域 -->
    <div v-if="store.status === 'settings'" class="settings-card card animate-fade-in">
      ...
      <div class="setting-item">
        <label>题目类型</label>
        <div class="options-group">
          <button :class="{ active: store.settings.type === '2-num' }" @click="store.settings.type = '2-num'">2个数运算</button>
          <button :class="{ active: store.settings.type === '3-num' }" @click="store.settings.type = '3-num'">3个数运算</button>
          <button :class="{ active: store.settings.type === 'priority' }" @click="store.settings.type = 'priority'">带优先级</button>
        </div>
      </div>
    ...
    <div v-if="store.status === 'practicing'" class="game-area animate-fade-in">
      ...
      <div class="keypad">
        <div class="keys">
          <button v-for="n in 9" :key="n" @click="handleKeypad(n.toString())">{{ n }}</button>
        ...
        <div class="history-list">
          <div v-for="rec in store.history" :key="rec.id" class="history-item">
            <div class="info">
              ...
              <div class="config">{{ rec.settings.range }}以内 | {{ rec.settings.type }}</div>
            </div>
            <div class="stats">
